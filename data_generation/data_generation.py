...
class ReplayBuffer:
    ...
    def generate_data(self, policy):
        ...
        for _ in tqdm(range(self.size // n_envs)):
            actions = torch.tanh(policy(torch.from_numpy(observations).to(torch.float32).to(self.device)).sample()).to("cpu").numpy()
            new_observations, rewards, dones, infos = env.step(actions)
            for i in range(n_envs):
                # s, a, r, s_prime
                self.append([observations[i], actions[i], rewards[i], new_observations[i]])
